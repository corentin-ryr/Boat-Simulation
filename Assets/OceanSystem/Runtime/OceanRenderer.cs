...
namespace OceanSystem
{
    ...
    public class OceanRenderer : MonoBehaviour
    {
        ...
        // Diagnostic flags — kept until invisible-water root cause is identified, then removed.
        private bool _loggedSetupFailure;
        private bool _loggedGlobalsSet;
        ...
        private bool Setup()
        {
            if (!_material || !_colorsPreset)
            {
                ...
                {
                    Debug.LogError($"[OceanRenderer] Setup failed: material={(_material ? _material.name : "NULL")}, colorsPreset={(_colorsPreset ? _colorsPreset.name : "NULL")}. " +
                        $"No ocean globals will be set. Assign these on the Ocean prefab.", this);
        ...
        private void SetGlobalColorVariables()
        {
            ...
            if (!_loggedGlobalsSet)
            {
                Debug.Log($"[OceanRenderer] Globals set — Diffuse={(Color)_colorsPreset.Diffuse}, " +
                    $"DeepScatter={(Color)_colorsPreset.DeepScatter}, " +
                    $"ShallowScatter={(Color)_colorsPreset.ShallowScatter}, " +
                    $"ReflectionMask={(Color)_colorsPreset.ReflectionMask}, " +
                    $"reflectionsMode={_reflectionsMode}, " +
                    $"specCubeSource={(_reflectionsMode == OceanReflectionsMode.RealtimeProbe && _probe ? _probe.name : _reflectionsMode == OceanReflectionsMode.Custom && _cubemap ? _cubemap.name : "ReflectionProbe.defaultTexture")}", this);
                _loggedGlobalsSet = true;
            }
        }
